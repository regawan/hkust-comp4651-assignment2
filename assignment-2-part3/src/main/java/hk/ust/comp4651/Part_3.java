...
public class Part_3 {
    ...
    public static void delEmptyFilesRecursive(Path root) throws IOException {
      ...
      for (FileStatus st : fs.listStatus(root)) {
        Path stPath = st.getPath();
        // Goes to deepest directory recursively and then deletes its way through recursively
        if (st.isDirectory()) {
            delEmptyFilesRecursive(stPath);
        } else {
          // Only deletes empty files
          if (st.getLen() == 0) {
